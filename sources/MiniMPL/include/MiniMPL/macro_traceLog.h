...
#define ALWAYS_LOG_HERE_INFO(X)		    { outputTxt(X); ALWAYS_LOG_HERE(); }
    ...
    #define  ALWAYS_COMPILE_MSG(X)  __pragma(message ( X "\r\n" HEREINFO_POSA() ))      
#else
    #define  ALWAYS_COMPILE_MSG(X)  __pragma(message("[Compiling " __FUNCTION__ "] " X))   
#endif
...
#define CheckMacroExpand(X)     __pragma(message(X))     //used to check macro extension
